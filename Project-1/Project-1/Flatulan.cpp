...
void Flatulan::move()
{
    ...
    int dir = randInt(0, NUMDIRS-1);  // dir is now UP, DOWN, LEFT, or RIGHT (0,1,2,3)
    int r = m_row;
    int c = m_col;
    m_city->determineNewPosition(r, c, dir);
    if ( ! m_city->isPlayerAt(r, c)) // function checks if player in position; if not, update position of flatulan
    {
        m_row = r;
        m_col = c;
    }
